<?php
	...
	*/
	...
	//Ausgabe
	//Header
	header('Content-disposition: attachment; filename=xml_backup_personal_'.date("Ymd").'.xml');
	...
	if(mysql_num_rows($data))
		while($row=mysql_fetch_assoc($data))
		{
			...
			if($n[0])
				...
				<type>".$lang['ntypes'][$number['ntid']]."</type>
			...
			if($a[0])
				...
				<country>".$countries[$address['state_code']]['name']."</country>
				<countryISO2>".$countries[$address['state_code']]['iso2']."</countryISO2>
				<countryISO3>".$countries[$address['state_code']]['iso3']."</countryISO3>
				<countryCode>".$countries[$address['state_code']]['num_code']."</countryCode>
			</address>\n";
						$addressID++;
					}
			...
			if($s[0])
				...
				<type>".$stypes[$se['soctid']]['socialtype']."</type>
